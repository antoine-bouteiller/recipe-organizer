...
- **GUD-002**: Build the category control by reusing existing primitives: a local Base UI collapsible (inlined in
  `search.tsx`) for the reveal and the shared `Select` (`@/components/ui/select`) in `multiple` mode for the
