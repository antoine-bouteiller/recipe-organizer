...
const badgeVariants = cva(
  `
    relative inline-flex shrink-0 items-center justify-center gap-1 rounded-sm
    border border-transparent font-medium whitespace-nowrap transition-shadow
    outline-none
  ...
  `,
  {
    ...
    variants: {
      size: {
        ...
        sm: `
          rounded-[calc(var(--radius-sm)-2px)] px-[calc(--spacing(1)-1px)]
          text-[.625rem]
        `,
      },
      variant: {
...
const Badge = ({ className, render, size, variant, ...props }: BadgeProps) => {
  const defaultProps = {
