...
import { Select, SelectButton, SelectItem, SelectPopup, SelectTrigger, type SelectButtonProps } from '@/components/ui/select'
...
interface ResponsiveSelectBaseProps {
  items: ResponsiveSelectOption[]
  placeholder?: string
  title?: string
  disabled?: boolean
  className?: string
...
type ResponsiveSelectProps = ResponsiveSelectBaseProps &
  (
...
export const ResponsiveSelect = (props: ResponsiveSelectProps): ReactElement => {
  ...
  return (
    <Select disabled={disabled} items={items} onValueChange={props.onValueChange} value={props.value ?? null}>
      {trigger}
      {popup}
    </Select>
  )
