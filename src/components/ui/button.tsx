...
const buttonVariants = cva(
  `
    ...
    gap-2 rounded-lg border bg-clip-padding text-sm font-medium
    whitespace-nowrap transition-shadow outline-none
  ...
  `,
  {
    ...
    variants: {
      size: {
        ...
        sm: `
          min-h-7 gap-1.5 px-[calc(--spacing(2.5)-1px)]
          py-[calc(--spacing(1)-1px)]
        `,
        xl: `
          min-h-10 px-[calc(--spacing(4)-1px)] py-[calc(--spacing(2)-1px)]
          text-base
          [&_svg:not([class*='size-'])]:size-4.5
        `,
        xs: `
          min-h-6 gap-1 rounded-md px-[calc(--spacing(2)-1px)]
          py-[calc(--spacing(1)-1px)] text-xs
          before:rounded-[calc(var(--radius-md)-1px)]
      ...
      variant: {
        ...
        `,
        'destructive-outline': `
          border-border bg-transparent text-destructive-foreground shadow-xs
          ...
          dark:not-disabled:before:shadow-[0_-1px_--theme(--color-white/4%)]
...
const Button = ({ className, render = <button />, size, variant, ...props }: ButtonProps) => {
