...
const inputGroupAddonVariants = cva(
  `
    flex h-auto cursor-text items-center justify-center gap-2 select-none
    not-has-[button]:**:[svg]:opacity-72
...
const InputGroupText = ({ className, ...props }: React.ComponentProps<'span'>) => (
  <span
    className={cn(`flex items-center gap-2 text-muted-foreground [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4`, className)}
    {...props}
  />
)

const InputGroupInput = ({ className, ...props }: InputProps) => <Input className={className} unstyled {...props} />

const InputGroupTextarea = ({ className, ...props }: TextareaProps) => <Textarea className={className} unstyled {...props} />
