...
} from "@/stores/shopping-list.store";
...
const RecipePage = () => {
  ...
  const { quantity, decrementQuantity, incrementQuantity } =
    useRecipeQuantities(recipe?.id, recipe?.quantity);
      ...
      <div className="md:max-w-5xl w-full relative md:shadow-sm md:bg-card md:border md:rounded-2xl h-fit">
        ...
        <div className="p-0">
          <div className="mb-6 w-full overflow-hidden md:rounded-t-md flex items-center justify-center aspect-16/6">
            <img
              src={getFileUrl(recipe.image)}
              alt={recipe.name}
              className="object-cover w-full"
            />
          </div>
          <h1 className="px-6 font-semibold text-xl md:text-2xl">
            {recipe.name}
          </h1>
