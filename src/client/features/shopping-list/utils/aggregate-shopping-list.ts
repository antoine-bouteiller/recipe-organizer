...
import { scaleQuantity } from '@shared/utils/scale-quantity'
...
export interface ShoppingListRecipe {
  ...
  readonly ingredients: readonly ShoppingListIngredient[]
}
...
): Map<number, IngredientAccumulator> => {
...
const inflateParents = (
  ingredientsMap: Map<number, IngredientAccumulator>,
  aggregatesById: Map<number, AggregatedIngredient>
...
const groupByCategory = (ingredients: AggregatedIngredient[]): Partial<Record<IngredientCategory, IngredientCartItem[]>> =>
  ingredients.reduce<Partial<Record<IngredientCategory, IngredientCartItem[]>>>((acc, ingredient) => {
...
export const aggregateShoppingList = (
  ...
  const ingredientsMap = accumulateIngredients(recipes, recipesQuantities)
  ...
  return groupByCategory(inflateParents(ingredientsMap, aggregatesById))
}
