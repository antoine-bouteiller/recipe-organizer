...
- `[C-3]` Search renders in the browser SPA; its route loader populates the query cache before the
  screen renders (`src/client/routes/search.tsx`).
...
(`src/client/features/search/utils/filter.ts:11-15`; `src/client/routes/search.tsx:39-40`).

When any filter is active, the page renders filtered results. Otherwise it passes only non-spice
rows to recents, maintaining the default spice-hiding policy (`src/client/routes/search.tsx:37-39`; `src/client/routes/search.tsx:94`).
...
(`src/client/features/search/utils/filter.ts:13-15`; `src/client/features/search/utils/filter.ts:37-38`).
...
(`src/client/features/search/components/search-results.tsx:16-55`). The source recipe query orders rows by
name, so the feature does not impose a competing sort (`src/client/features/recipe/api/get-all.ts:23-38`).
...
continues to capture the recent-recipe visit (`src/client/features/search/components/search-results.tsx:13-34`; `src/client/features/search/components/recipe-search-card.tsx:19-29`).
...
(`src/client/stores/recent-recipes.store.ts:5-14`). `RecentRecipes` resolves IDs in stored order against the
query result, omits stale IDs, and displays the supplied catalogue when resolution yields no rows
(`src/client/features/search/components/recent-recipes.tsx:19-37`).

The store exposes its array through a selector hook, keeping persistence mechanics outside the page
component (`src/client/stores/recent-recipes.store.ts:7-9`). The visible history header includes the French
clear control only when at least one recent row resolves (`src/client/features/search/components/recent-recipes.tsx:27-35`).
...
(`src/client/features/search/components/recipe-search-card.tsx:19-29`). The card renders the recipe image,
name, and attribute badges from the recipe projection (`src/client/features/search/components/recipe-search-card.tsx:30-59`).

The optional action slot lets filtered results add a recipe to the shopping list without changing the
navigation contract (`src/client/features/search/components/search-results.tsx:12-15`; `src/client/features/search/components/recipe-search-card.tsx:7-10`).
