...
recipe's preparation ordered groups of typed steps, serving `index.spec.md` [G-3]. It owns the step
...
- `[NG-3]` Letting a sub-recipe group select a recipe outside the form's linked recipes.
...
- `[PI-3]` **References remain live** — a sub-recipe group stores an id and fetches the source's
  default group rather than copying it.
...
const subrecipeStepGroupSchema = z.object({ kind: z.literal('subrecipe'), recipeId: z.number().int().positive() })
export type RecipeStepGroup = z.infer<typeof recipeStepGroupSchema> // union of both

// recipeSchema: `instructions: z.string()` becomes, with `_key` on every group and step,
stepGroups: z.array(ownGroup | subrecipeGroup).refine((groups) => groups[0]?.kind === 'steps')
```
...
- Each group numbers its steps from 1; an empty group shows no list.
- The source comes from the instructions projection (`display.spec.md` [KD-1]), which returns the
  source's default group; an unavailable source renders nothing.
- A default group is never a sub-recipe group, so embedded sections never recurse.
...
- `[VC-5]` Given source recipe B whose default group has 3 steps and recipe A with a sub-recipe group
  for B, when A's detail page renders, then B's 3 steps appear under B's name; after B's default
...
- `[C-5]` _Superseded by [KD-10]:_ sub-recipe groups carry no range.
- `[C-6]` A sub-recipe group shows only the source's default group, like linked-recipe ingredients.
