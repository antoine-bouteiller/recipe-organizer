...
export const cacheLimits = {
  ...
  publicAssets: { maxAgeFrom: 'last-used' as const, maxAgeSeconds: 30 * day, maxEntries: 60 },
...
export const isPublicRecipeRequest = ({ request, sameOrigin, url }: { request: Request; sameOrigin: boolean; url: URL }) =>
  sameOrigin && request.method === 'GET' && /^\/api\/recipes(?:\/\d+(?:\/instructions)?)?\/?$/.test(url.pathname)
