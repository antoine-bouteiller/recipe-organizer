...
const rootRecipe = cva({
  base: { alignItems: 'center', backgroundColor: 'background', display: 'flex', justifyContent: 'center', marginBlock: 'auto' },
})
const stackRecipe = cva({ base: { display: 'flex', flexDirection: 'column', gap: '8', textAlign: 'center' } })
const centeredRecipe = cva({ base: { display: 'flex', justifyContent: 'center' } })
const markRecipe = cva({ base: { height: 'auto', width: '48' } })
const contentRecipe = cva({ base: { display: 'flex', flexDirection: 'column', gap: '4' } })
const headingRecipe = cva({ base: { color: 'foreground', fontSize: '2xl', fontWeight: 'semibold' } })
export interface NotFoundProps {
  action?: React.ReactNode
}
export const NotFound = ({ action }: NotFoundProps): React.ReactElement => (
  <div className={rootRecipe()}>
    <div className={stackRecipe()}>
      <div className={centeredRecipe()}>
        <svg width="199" height="73" viewBox="0 0 199 73" fill="none" xmlns="http://www.w3.org/2000/svg" className={markRecipe()}>
          ...
          />
          <defs>
            <linearGradient id="paint0_linear_2672_10455" x1="99" y1="-16" x2="99" y2="88" gradientUnits="userSpaceOnUse">
      ...
      <div className={contentRecipe()}>
        <h1 className={headingRecipe()}>Page introuvable</h1>
