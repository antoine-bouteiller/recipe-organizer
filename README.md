...
- **Styling**: vanilla-extract with colocated `.css.ts` files and shared pixel-based tokens, owned Base UI components in `@recipe-organizer/design-system`, Storybook
...
The app, Storybook, and existing tests compile colocated `.css.ts` files through the vanilla-extract Vite plugin. Shared pixel-based tokens and dark-theme values live in `packages/design-system/src/theme/tokens.css.ts`, exported as `@recipe-organizer/design-system/tokens` and loaded by the app and Storybook entrypoints. No separate style-generation step is needed. Components own private recipes and declare minimal local props; Base UI `render` composes existing components, including the actual router `Link`. Parent wrappers own external layout. Native global CSS remains for owned global and runtime-only behavior.
