---
title: Forms Specification (TanStack Form + Zod + Kobalte)
version: 1.0
date_created: 2026-05-08
last_updated: 2026-05-08
owner: recipe-organizer
tags: [infrastructure, forms, tanstack-form, zod, kobalte, validation]
---

# Introduction

This document specifies the forms infrastructure of the recipe-organizer application. The stack combines
`@tanstack/solid-form` (state, validation, field-array support), Zod schemas reused from the matching server-function
file (single source of truth), and framework-plain Solid `Form`/`Field` components that distribute errors via a
Solid context (error wiring, accessibility).
...
- The catalog of field components registered in the hook (`TextField`, `NumberField`, `SelectField`,
  `ComboboxField`, `CheckboxField`, `ToggleGroupField`, `ImageField`, `VideoField`, `EditorField`) and the
  framework-plain Solid `Field` components also exposed (`Field`, `FieldControl`, `FieldDescription`, `FieldError`, `FieldLabel`).
- The `FormSubmit` form-component and the `<Form>` wrapper (framework-plain Solid) used by every form.
...
- The `defaultValues` convention shared between the form view and the route component.
...
- `src/components/ui/form.tsx`, `src/components/ui/field.tsx` (framework-plain Solid components).
...
| Term                      | Definition                                                                                                                                                                                                                                                 |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TanStack Form             | `@tanstack/solid-form`. Provides `createFormHook`, `createFormHookContexts`, `useStore`, `revalidateLogic`, `createFieldMap`.                                                                                                                              |
...
| Field components          | Solid components registered in `fieldComponents` of `createFormHook`. Each component reads `useFieldContext<T>()` internally.                                                                                                                              |
| Form components           | Solid components registered in `formComponents` of `createFormHook`. Currently only `FormSubmit`.                                                                                                                                                          |
| `<Form>`                  | `src/components/ui/form.tsx` — framework-plain Solid component. Accepts `errors: Record<string, string>` and broadcasts them via `FormErrorsContext`.                                                                                                      |
| `<Field>`                 | `src/components/ui/field.tsx` — framework-plain Solid components (`Field`, `FieldLabel`, `FieldControl`, `FieldDescription`, `FieldError`, `FieldValidity`) built on a per-field Solid context.                                                            |
...
| `defaultValues`           | A `Partial<TFormInput>` (or `TFormInput`) constant declared next to the schema and passed to `useAppForm`. Subforms read state through the `useAppForm`/field contexts (accessor field API, `field().state.value`); no `createFieldMap` field-map is used. |
...
- **REQ-003** `useAppForm` SHALL register `FormSubmit` as a form component, accessible via `form.AppForm` >
  `form.FormSubmit`.
- **REQ-004** `EditorField` (`src/components/forms/editor-field.tsx`) SHALL import `{ Editor, EditorContent,
EditorToolbarButton }` directly from `@/components/common/editor` (a hand-rolled Solid Lexical binding). There is no
  lazy-loading and no `<Suspense>` boundary; the editor renders client-side under the app's client-only render mode.
...
- **REQ-007** Every form SHALL be wrapped in `<Form>` (`src/components/ui/form.tsx`). Page forms SHALL pass
  `errors={useStore(form.store, (state) => formatFormErrors(state.errors))}` so `<Form>` can broadcast errors via
  `FormErrorsContext` to child fields keyed by `name`. The `<Form>` `onSubmit` handler SHALL call
  `event.preventDefault()` then `void form.handleSubmit()`.
...
- **REQ-009** Each feature form SHALL declare a `defaultValues` constant whose type is `Partial<TFormInput>` (or
  `TFormInput` when every field is required). Subforms consume the form via `useAppForm`/field contexts (the accessor
  field API, `field().state.value`); a `createFieldMap` field-map is no longer required.
...
- **REQ-011** Dynamic field arrays SHALL use `<AppField mode="array" name={path}>` with the render-prop
  ...
  Each item SHOULD carry a stable client-side `_key` (random string) used as the list `key` (for Solid `<For>`/`<Index>`),
...
- **REQ-018** `EditorField` SHALL accept a `nodes?: readonly Klass<LexicalNode>[]` prop and an optional
  `extraToolbar` `JSX.Element` so feature-specific nodes (e.g. recipe Magimix/Subrecipe) can be plugged in without
...
- **CON-005** `<Form>` MUST wrap the form to broadcast errors via `FormErrorsContext`. Plain `<form>` does not
  propagate the `errors` map to descendant `<Field>` roots.
...
- **GUD-001** Reuse the server function's exported Zod schema and inferred input type. Do not redeclare the schema
  client-side.
- **GUD-002** Define `defaultValues` next to the schema (typically in `src/features/<feature>/utils/form.ts`) and
  pass `form={form}` to subforms.
...
- **GUD-005** Use `_key: Math.random().toString(36).substring(7)` for client-side keys in dynamic arrays. Do
  not key by database `id` — newly added rows have id `-1` until the server assigns one.
- **GUD-006** `EditorField` imports its Solid editor directly (no lazy-loading). Under the app's client-only render
  mode the editor renders client-side, so no SSR/`<Suspense>` fallback is needed.
...
- **PAT-007 (Error projection)** `const errors = useStore(form.store, (s) => formatFormErrors(s.errors))` ->
  `<Form errors={errors}>`. `<Form>` broadcasts `errors` via `FormErrorsContext`; each `<Field name>` reads its
  entry by `name` and renders it via `<FieldError />`.

## 4. Interfaces & Data Contracts

### 4.1 Field Component Catalog

| Component          | Field value type                | Underlying primitive (Kobalte / project)                                        | Key props                                                                                                                                         |
| ------------------ | ------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| `SelectField`      | `string \| undefined`           | `Select*` (`components/ui/select`, Kobalte Select)                              | `items: { label: string; value: string }[]`, `label?`, `disabled?`                                                                                |
| `ComboboxField`    | `number \| string \| undefined` | `Combobox*` (desktop) / `Drawer*` (mobile)                                      | `options: Option<TValue>[]`, `addNew?: (input: string) => JSX.Element`, `label?`, `placeholder?`, `searchPlaceholder?`, `disabled?`, `className?` |
...
| `EditorField`      | `string` (Lexical JSON)         | Solid Lexical `Editor` (`components/common/editor`, direct import)              | `label?`, `nodes?: readonly Klass<LexicalNode>[]`, `extraToolbar?: JSX.Element`, `disabled?`                                                      |

Framework-plain Solid `Field*` components (`src/components/ui/field.tsx`) also exposed under `form.AppField`'s
second argument (same roles as before, built on a per-field Solid context):
...
| `FieldError`       | Error message slot. Reads its entry from `FormErrorsContext` keyed by the field's `name`.    |
...
| `withForm({ defaultValues, props, render })` | Returns a Solid component `(props & { form }) => JSX`. Used for both subforms and `getFormDialog`. |

### 4.3 `<Form>` and `<Field>` (framework-plain Solid components)

| Component          | Source                        | Notes                                                                                                                                                                 |
| ------------------ | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Form`             | `src/components/ui/form.tsx`  | Class default `flex w-full flex-col gap-4`. Accepts `errors: Record<string, string>`; provides `FormErrorsContext` (a `createContext<() => Record<string, string>>`). |
| `Field` (Root)     | `src/components/ui/field.tsx` | Class default `flex flex-col items-start gap-2 w-full`. Establishes a per-field Solid context.                                                                        |
| `FieldLabel`       | `src/components/ui/field.tsx` | Cursor pointer when wrapping a Checkbox.                                                                                                                              |
| `FieldControl`     | `src/components/ui/field.tsx` | Native control slot, no class default.                                                                                                                                |
| `FieldItem`        | `src/components/ui/field.tsx` | Class default `flex`.                                                                                                                                                 |
| `FieldDescription` | `src/components/ui/field.tsx` | Muted helper text.                                                                                                                                                    |
| `FieldError`       | `src/components/ui/field.tsx` | Destructive text. Reads its entry from `FormErrorsContext` by the field's `name`.                                                                                     |
| `FieldValidity`    | `src/components/ui/field.tsx` | Field validity slot.                                                                                                                                                  |
...
The route imports `recipeDefaultValues`, passes it to `useAppForm`, and passes `form={form}` to the subform. Subforms
read state through the `useAppForm`/field contexts (the accessor field API, `field().state.value`); no
`createFieldMap` field-map is used.
...
- **AC-010** `EditorField` imports its Solid editor directly from `@/components/common/editor` (no lazy-loading, no
  `<Suspense>` boundary) and renders client-side under the app's client-only render mode.
...
- **AC-021** Removing an item via `field.removeValue(index)` removes the matching subtree without losing
...
- **Integration**:
  ...
  - Dynamic array: programmatically push/remove ingredient rows; assert `state.values.ingredientGroups[i].ingredients`
    matches expectations and that keys are stable.
...
- **Field components own the primitive wiring.** Centralizing `useFieldContext` access inside each component means
  ...
  shape is defined.
- **Errors via context broadcast.** TanStack Form's `state.errors` is a list of issue maps. Reducing it to
  `Record<string, string>` and feeding it to `<Form errors>` lets `<Form>` broadcast the map through
  `FormErrorsContext`; each `<Field>` reads its entry by `name` and handles DOM wiring (`aria-describedby`,
...
- **`defaultValues` next to the schema.** Keeping `defaultValues` beside the schema documents the form shape at a
  glance; subforms read state through the `useAppForm`/field contexts (accessor field API) rather than a
  `createFieldMap` field-map.
...
- `@/components/ui/{form,field,input,number-input,select,combobox,checkbox,toggle-group,toolbar,kbd,spinner,button,separator,skeleton,drawer,responsive-dialog}`.
- `@/components/common/editor` (Solid Lexical binding used by `EditorField`).
...
- Feature `defaultValues` modules: `src/features/<feature>/utils/form.ts`.

### 8.2 External

- `@tanstack/solid-form` (`createFormHook`, `createFormHookContexts`, `useStore`, `revalidateLogic`,
  `withForm`, `StandardSchemaV1Issue`).
- `@kobalte/core` (primitives underlying `Select` and other UI components). `Form`/`Field` are framework-plain
  Solid components with no external primitive library.
- `zod` (schemas authored alongside server functions).
- `lexical` (typed `Klass<LexicalNode>` for `EditorField` `nodes` prop).
- `unplugin-icons` (icons imported as `~icons/ph/<Name>` inside `ImageField`, `VideoField`, `EditorField` toolbar, dynamic-array buttons).
- `@tanstack/solid-router` (route components consuming the page-form pattern).
...
1. **Define the Zod schema in the server-function file** (e.g. `src/features/<feature>/api/create.ts`):
   ...
   ```
2. **Define `defaultValues`** (e.g. `src/features/<feature>/utils/form.ts`):
   ```
...
5. **Pipe errors via `formatFormErrors`** — `const errors = useStore(form.store, (s) => formatFormErrors(s.errors))`,
   pass `errors` to `<Form>`. `<Form>` broadcasts the map via `FormErrorsContext`; each `<Field name>` reads its
   entry and renders it through its `<FieldError />` slot.
...
- **`undefined` array path**: `field.state.value` for an `<AppField mode="array">` may be `undefined`. Always guard
  with `?.map(...)`.
- **Numeric ids in dynamic arrays**: New rows use `id: -1` until persisted; keys MUST come from a separate
  client-side `_key` to avoid duplicate-key warnings while the user adds two new rows in a row.
- **Editor render mode**: `EditorField` imports its Solid editor directly (no lazy-loading, no `<Suspense>`). Under
  the app's client-only render mode the editor renders client-side, so there is no SSR fallback to size.
...
- **VAL-006** No field component imports `useForm`, `createFormHook`, or `useStore` from `@tanstack/solid-form`
...
- Kobalte (primitives underlying Select and other UI components): https://kobalte.dev/. `Form`/`Field` are
  framework-plain Solid components with no external primitive library.
