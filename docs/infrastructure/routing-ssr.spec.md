...
- **REQ-012** — `/settings/index` MUST hide admin-only sections by filtering `settingsSections` on
  `context.isAdmin`. `/settings/users` is rendered only when `isAdmin` (UI gating in
  `/settings/index`); the route component itself does NOT re-check, so the admin gate is
  presentation-level on the index plus auth-level via the parent `/settings` `beforeLoad`.
...
| `/settings/users`           | `routes/settings/users.tsx`          | Inherits + Admin (UI gate on `/settings/index`) | `getUserListOptions('active')`, `('blocked')`, `('pending')`                          | —                                                     | —                                                      |
...
- `/settings/users` is reachable directly by URL even by non-admins (its `beforeLoad` only checks
  auth via the parent `/settings`). The admin gate is presentation-level on `/settings/index`'s
  card list. Tighten if needed.
